...
st.markdown("<h1 style='color: blue; font-weight: bold;'>Image Search By Style</h1>", unsafe_allow_html=True)

# Load the TensorFlow model
model = tf.keras.models.load_model("jeans.keras")

def style_to_vec(style):
    # Flatten and convert the style tensor to a NumPy array
    return style.numpy().ravel()

image_paths = glob.glob('/Users/svs/Desktop/Projects/ADM/Assignment-3/jeans_images/*.jpg')
...
if query_image is not None:
    ...
    query_image = query_image.convert("RGB")  # Convert to RGB mode if necessary
    query_image = query_image.resize((400, 400))  # Resize the image
    query_image = np.array(query_image)  # Convert to NumPy array
    query_image = tf.convert_to_tensor(query_image, dtype=tf.float32)
